...
//session and Term
        ...
        $qurty=mysqli_query($conn,"select * from tblattendance  where classId = '$_SESSION[classId]' and classArmId = '$_SESSION[classArmId]' and subject = '$_SESSION[subject]' and dateTimeTaken='$dateTaken'");
        ...
        if($count == 0){ //if Record does not exsit, insert the new record
          ...
          {
              $qquery=mysqli_query($conn,"insert into tblattendance(admissionNo,classId,classArmId,sessionTermId,status,dateTimeTaken,subject) 
              value('$ros[admissionNumber]','$_SESSION[classId]','$_SESSION[classArmId]','$sessionTermId','0','$dateTaken','$_SESSION[subject]')");
...
//check if the attendance has not been taken i.e if no record has a status of 1
  $qurty=mysqli_query($conn,"select * from tblattendance  where classId = '$_SESSION[classId]' and classArmId = '$_SESSION[classArmId]' and dateTimeTaken='$dateTaken' and subject = '$_SESSION[subject]' and status = '1'");
  $count = mysqli_num_rows($qurty);
